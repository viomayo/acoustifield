...
- Exports JSON et CSV (UTF-8 avec BOM, séparé par des points-virgules pour Excel, avec `user_id`/`user_name`, `nb_photos` et la colonne `photos` listant les noms des fichiers liés), plus export ZIP des photos d'une fiche. Les fichiers téléchargés sont nommés « Projet - Date - Site - Opérateur » et les photos « Projet - Date - Site - Opérateur - NN.jpg ».
