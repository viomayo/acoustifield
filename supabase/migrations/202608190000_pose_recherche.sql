-- AcoustiField: remplacer date_debut_nuit par date_heure_pose, date_heure_recherche, nb_nuits_ecoute.
-- À appliquer APRÈS 202608150001_orientation_deg.sql.

alter table public.fiches add column if not exists date_heure_pose timestamptz;
alter table public.fiches add column if not exists date_heure_recherche timestamptz;
alter table public.fiches add column if not exists nb_nuits_ecoute integer;

-- Migrer les données existantes : date_debut_nuit (date) → date_heure_pose (timestamptz)
update public.fiches set date_heure_pose = date_debut_nuit::timestamptz where date_debut_nuit is not null and date_heure_pose is null;

alter table public.fiches drop column if exists date_debut_nuit;
...
begin
  ...
  insert into public.fiches (
    ...
    projet, operateur, date_heure_pose, date_heure_recherche, nb_nuits_ecoute,
    site_nom, lat, lon, commune,
    ...
    gestion, eclairage, hauteur_pose_m, orientation_deg, temperature_c,
    ...
    nullif(p_snapshot -> 'fiche' ->> 'date_heure_pose', '')::timestamptz,
    nullif(p_snapshot -> 'fiche' ->> 'date_heure_recherche', '')::timestamptz,
    nullif(p_snapshot -> 'fiche' ->> 'nb_nuits_ecoute', '')::integer,
    ...
    nullif(p_snapshot -> 'fiche' ->> 'orientation_deg', '')::double precision,
    ...
    date_heure_pose = excluded.date_heure_pose,
    date_heure_recherche = excluded.date_heure_recherche,
    nb_nuits_ecoute = excluded.nb_nuits_ecoute,
